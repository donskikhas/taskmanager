...
const DocEditor: React.FC<DocEditorProps> = ({ doc, onSave, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F0F2F5] dark:bg-notion-dark-bg overflow-hidden">
      {/* Top Bar */}
      <div className="bg-white dark:bg-notion-dark-sidebar border-b border-gray-200 dark:border-notion-dark-border px-4 py-2 flex items-center justify-between shrink-0 shadow-sm z-10">
        <div className="flex items-center gap-3">
            <button 
                onClick={onBack}
                className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 p-2 hover:bg-gray-100 dark:hover:bg-notion-dark-hover rounded-full transition-colors"
                title="Назад"
            >
                <ArrowLeft size={20} />
            </button>
            <div className="flex flex-col">
                <input 
                    ...
                    className="font-bold text-lg text-gray-800 dark:text-white border-none p-0 focus:ring-0 placeholder-gray-300 bg-transparent w-64 lg:w-96"
        ...
        <div className="flex items-center gap-2">
            <button 
                ...
                className="flex items-center gap-2 px-3 py-1.5 text-gray-600 dark:text-gray-300 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md text-sm transition-colors"
            >
                <Download size={16} /> <span className="hidden sm:inline">Экспорт</span>
            </button>
            <button 
                onClick={handleSave}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all shadow-sm ${isSaved ? 'bg-green-100 text-green-700 border border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
            >
      ...
      {/* Toolbar */}
      <div className="bg-white dark:bg-notion-dark-sidebar border-b border-gray-200 dark:border-notion-dark-border px-4 py-2 flex items-center justify-center gap-1 shrink-0 shadow-sm z-10 overflow-x-auto">
...
const ToolbarBtn: React.FC<{icon: React.ReactNode, onClick: () => void, title: string}> = ({icon, onClick, title}) => (
    <button 
        ...
        className="p-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-notion-dark-hover rounded transition-colors"
        title={title}
    >
        {icon}
    </button>
